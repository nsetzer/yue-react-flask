    ...
    for i, part in enumerate(parts):
        if part.startswith(":"):
            ...
            try:
                ...
                error = "error formating variable: %s: %s" % (varname, e)
...
class AuthenticatedRestClient(object):
    ...
    def __init__(self, host, username, password, domain=None, role=None):
...
class ApplicationClient(object):
    def __init__(self, endpoints):
        ...
        for endpoint in endpoints:
            name = endpoint.long_name \
                ...
                .replace("resource", "") \
                .replace(".", "_") \
                .replace("filesys_", "files_") # todo: fixme
            #print(name)
    ...
    def connect(self, hostaddr, username, password, domain=None, role=None):
        self._client = AuthenticatedRestClient(hostaddr,
            username, password, domain, role)

    def __getattr__(self, attr):

        if self.__dict__['_client'] is None:
