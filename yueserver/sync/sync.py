...
class LocalStorageDao(object):
    ...
    def search(self, terms, path_prefix=None, limit=None, offset=None, delimiter='/'):
        ...
        if path_prefix:
            ...
            where = FsTab.c.rel_path.startswith(bindparam('path_prefix', path_prefix))

        if terms:
            rules = [self.grammar.ruleFromString(term) for term in terms]
...
class DirEnt(object):
    ...
    def samefile(self, other):
        ...
        x = None
        if self.local_base and other.local_base:
            x = self.local_base == other.local_base

        y = None
        if self.remote_base and other.remote_base:
            y = self.remote_base == other.remote_base
...
class FileEnt(object):
    ...
    def samefile(self, other):
        if not isinstance(other, FileEnt):
            return False

        x = None
        if self.local_path and other.local_path:
            x = self.local_path == other.local_path

        y = None
        if self.remote_path and other.remote_path:
            y = self.remote_path == other.remote_path
...
def _check_iter(ctxt, remote_base, local_base):
    ...
    try:
        if not ctxt.fs.islocal(local_base) or ctxt.fs.exists(local_base):
            _records_iter = ctxt.fs.scandir(local_base)
    except NotADirectoryError:
        pass

    _name2dirs = {d:None for d in dirs}
    _name2files = {f.name:f for f in _files}

    for record in _records_iter:
        pass

def _check_dir2ent(ctxt, attr, remote_base, local_base, d, record):
    # d: the directory name
        ...
        if attr.match(d):
            if ctxt.showHiddenNames:
                ...
                # attr blocked this record from being listed
                return None
...
def _check_file2ent(ctxt, remote_base, local_base, f, record):

    # f:FileRecord{.name, .isDir, .size, .mtime, .version, .permission, .encrypted, .storage_path, .file_path}

    name = f['rel_path']
    remote_path = posixpath.join(remote_base, f['rel_path'])
    local_path = ctxt.fs.join(local_base, f['rel_path'])

    if record and record.isDir:
        state = FileState.CONFLICT_TYPE
        return DirEnt(name, remote_path, local_path, state)
    ...
    try:
        if record:
            #st = ctxt.fs.file_info(local_path)
    ...
    return FileEnt(remote_path, local_path, lf, rf, af)
...
def _check(ctxt, remote_base, local_base):
    ...
    for d in _dirs:
        ...
        ent = _check_dir2ent(ctxt, attr, remote_base, local_base, d, _names.get(d, None))
    ...
    for f in _files:
        ...
        ent = _check_file2ent(ctxt, remote_base, local_base, f, _names.get(name, None))
...
def _sync_file_impl(ctxt, ent, push=False, pull=False, force=False):
    ...
    if FileState.SAME == state:
        ...
        sys.stdout.write("use force to push %s\n" % ent.remote_path)
         ...
         (FileState.PULL == state and push and force and ent.af) or \
        ...
        sys.stdout.write("use force to pull %s\n" % ent.remote_path)
         ...
         (FileState.PUSH == state and pull and force and ent.rf) or \
...
def _remove_impl_local_dir(ctxt, ent):
    """remove db entry and local file"""

    if not os.listdir(ent.local_base):
        ctxt.fs.rmdir.rmdir(ent.local_base)
        return True
...
def _remove_impl(ctxt, ents, local, remote):
    ...
    # for local directories attempt to remove empty directories
    # process in reverse because deeper nodes will be appended last
    if local:
        for ent in reversed(dirs):
            _remove_impl_local_dir(ctxt, ent)
...
def _search_impl(ctxt, terms, limit, offset):

    results = ctxt.storageDao.search(
        terms,
        path_prefix=ctxt.current_remote_base,
        limit=limit,
        offset=offset)
...
class SearchCLI(object):
    ...
    def register(self, parser):
        ...
        subparser.add_argument("-n", "--limit", default=None,
            help="limit search results")

        subparser.add_argument("--offset", default=None,
            help="skip first offset results.")

        subparser.add_argument('terms', nargs="*", help="search terms")

    def execute(self, args):
        ctxt = get_ctxt(os.getcwd())
        ctxt.verbose = args.verbose + 1

        _search_impl(ctxt, args.terms, args.limit, args.offset)
